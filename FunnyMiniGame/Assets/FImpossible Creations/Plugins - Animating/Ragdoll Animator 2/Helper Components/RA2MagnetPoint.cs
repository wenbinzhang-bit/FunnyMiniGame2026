...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public class RA2MagnetPoint : FimpossibleComponent
    {
        ...
        [FPD_SingleLineTwoProps("RestoreOnDestroy")]
        public bool KinematicOnMax = false;
        [Tooltip("Restore rigidbody's .IsKinematic to the right value on magnet object destroy or disable")]
        [HideInInspector] public bool RestoreOnDestroy = true;
        ...
        private void OnDisable()
        {
            ...
            if (RestoreOnDestroy == false) return;
            
            // If ragdoll handler and ragdoll bone is provided, use bone specific settings for isKinematic switch
            if (attachBone != null && attachBone.BypassKinematicControl == false && handler != null)
            {
                var rHandler = handler.GetRagdollHandler;

                if (rHandler != null)
                {
                    if (attachBone.ForceKinematicOnStanding && rHandler.IsInStandingMode)
