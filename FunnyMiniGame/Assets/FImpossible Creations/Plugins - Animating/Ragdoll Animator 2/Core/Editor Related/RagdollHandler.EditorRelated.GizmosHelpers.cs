...
namespace FIMSpace.FProceduralAnimation
{
    public partial class RagdollHandler
    {
        ...
        public void EditorHandles_RagdollDrawBoneChain( RagdollBonesChain chain, float alpha = 0.6f )
        {
            ...
            {
                if (lastBone.SourceBone.parent)
                {
                    //Vector3 nextPos = lastBone.SourceBone.position + (lastBone.SourceBone.position - lastBone.SourceBone.parent.position) * 0.65f;
                    Vector3 localMain = lastBone.GetMainAxis();
                    Vector3 localSecondary = lastBone.GetSecondaryAxis();
                    Vector3 localBoneDir = Vector3.Cross(localMain, localSecondary).normalized;
                    float size = (lastBone.SourceBone.position - lastBone.SourceBone.parent.position).magnitude;

                    Vector3 nextPos = lastBone.SourceBone.position + (localBoneDir * size * (lastBone.InverseMainAxis ? -1 : 1));

                    FGUI_Handles.DrawBoneHandle(lastBone.SourceBone.position, nextPos, .6f * boneWdth, false, 3, .8f, 4f, 0.4f);
                }
            }
