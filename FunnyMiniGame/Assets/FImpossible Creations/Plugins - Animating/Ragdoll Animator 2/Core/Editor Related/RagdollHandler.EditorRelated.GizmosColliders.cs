...
namespace FIMSpace.FProceduralAnimation
{
    public partial class RagdollHandler
    {
        ...
        private void EditorHandler_DrawColliderAdjustementsHandles(RagdollChainBone bone, RagdollChainBone.ColliderSetup setup)
        {
            ...
            Vector3 handlesScale = bone.SourceBone.lossyScale;
            float handlesRescaller = bone.SourceBone.lossyScale.x;

            if (RagdollHandler.AutoScaleHandles == false) { handlesScale = Vector3.one; handlesRescaller = 1f; avgSize = setup.Editor_GetHandleSize(bone) * 1.1f; }

            Handles.matrix = bone.GetMatrix(Vector3.zero, handlesScale, setup.RotationCorrectionQ);
            float selectSize = setup.Editor_GetHandleSize(bone) * (_editScale ? 6f : 3.5f) / handlesRescaller;
            float handleSize = setup.Editor_GetHandleSize(bone) * (_editScale ? 4f : 1.25f) / handlesRescaller;
            ...
            if (!_editScale)
            {
                Vector3 handlePos = setup.ColliderCenter;

                if (RagdollHandler.AutoScaleHandles)
                    handlePos = FEditor_TransformHandles.PositionHandle(setup.ColliderCenter, Quaternion.identity, avgSize, false, false);
                else
                {
                    var preMx = Handles.matrix;
                    Handles.matrix = Matrix4x4.identity;
                    handlePos = bone.SourceBone.TransformPoint(handlePos);
                    handlePos = FEditor_TransformHandles.PositionHandle(handlePos, Quaternion.identity, 0.5f, false, false);
                    handlePos = bone.SourceBone.InverseTransformPoint(handlePos);
