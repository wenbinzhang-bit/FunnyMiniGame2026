    ...
    public partial class RagdollHandler
    {
        ...
        public void EditorHandles_RagdollDrawBonePhysics( RagdollBonesChain chain, float alpha = 0.75f )
        {
            ...
            for( int b = 0; b < chain.BoneSetups.Count; b++ )
            {
                ...
                if( !IsSkinBone( nextBone, chain, Mecanim ) ) nextBone = null; // Thank you !leanon (discord): If bone is not used by avatar, we will not draw continous line to next bone, because it can be placed anywhere and it can cause confusion
                ...
                if( nextBone == null ) 
                {
                    ...
                    {
                        Vector3 localMain = bone.GetMainAxis();
                        Vector3 localSecondary = bone.GetSecondaryAxis();
                        Vector3 localBoneDir = Vector3.Cross(localMain, localSecondary).normalized;
                        float size = (bone.SourceBone.position - bone.SourceBone.parent.position).magnitude;

                        nextPos = bone.SourceBone.position + (localBoneDir * size * (bone.InverseMainAxis ? -1 : 1));
                        //nextPos = bone.SourceBone.position + ( bone.SourceBone.position - bone.SourceBone.parent.position ) * 0.65f;
                    }
                }
        ...
        static Transform _lastSkinsTransform = null;
        static System.Collections.Generic.List<SkinnedMeshRenderer> cachedSkinnedMeshRenderers = null;
        static bool IsSkinBone( Transform candidate, RagdollBonesChain chain, Animator mecanim )
        {
            Transform rigRoot = null;

#if UNITY_6000_0_OR_NEWER
            if ( mecanim != null) rigRoot = mecanim.avatarRoot;
            if ( rigRoot == null) rigRoot = chain.ParentHandler.GetBaseTransform();
#else
            rigRoot = chain.ParentHandler.GetBaseTransform();
#endif

            if( !candidate || !rigRoot ) return false;

            if ( rigRoot != _lastSkinsTransform || cachedSkinnedMeshRenderers == null)
            {
                _lastSkinsTransform = rigRoot;
               cachedSkinnedMeshRenderers = FTransformMethods.FindComponentsInAllChildren<SkinnedMeshRenderer>(rigRoot);
            }

            if( cachedSkinnedMeshRenderers.Count == 0 ) return true; // No renderers found - default return true

            foreach( SkinnedMeshRenderer smr in cachedSkinnedMeshRenderers )
            {
                var bones = smr.bones;
                ...
                {
                    if( bones[i] == candidate ) return true;
                }

                if( smr.rootBone == candidate ) return true;
            }
