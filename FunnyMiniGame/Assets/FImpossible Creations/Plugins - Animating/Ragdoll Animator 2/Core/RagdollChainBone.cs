...
namespace FIMSpace.FProceduralAnimation
{
    ...
    public class RagdollChainBone
    {
        ...
        public void SwitchPhysics( bool enable )
        {
            ...
            {
                // Null guard added: collider setup may have no generated GameCollider during teardown
                if( collider.GameCollider != null ) collider.GameCollider.enabled = enable;
            }

            // Null guards added: synthetic anchor bone has no ParentChain/Joint (same guard as enable path above)
            if (enable == false)
            {
                if (ParentChain != null && ParentChain.ParentHandler != null && ParentChain.ParentHandler.HardDisable && Joint != null) Joint.gameObject.SetActive(false);
            }
            else if (Joint != null && Joint.gameObject.activeInHierarchy == false)
            {
